...
def drawHist_02(h0, h1, h2, h3, v0, v1, v2, v3, fname):
    ...
    ind = np.arange(10)
    # length = int(min(len(y0), len(y1)))
    indb = np.arange(0.0, 10.0, 1)
    indc = np.arange(0.25, 10.25, 1)
    indd = np.arange(0.5, 10.5, 1)
    wid = 0.25
    maxv = max(max(h1), max(h2), max(h3), max(h0))
    print(len(indb), len(h1))
    pyplot.bar(indb, h1, wid, label="DCUM",
               color="w", edgecolor="orange", yerr=v1, capsize=2)
    pyplot.bar(indd, h2, wid, label="DUM",
               color="w", edgecolor="green", hatch='***', yerr=v2, capsize=2)
    # pyplot.bar(indd, h3, wid, label="LR", color="w",
    #            edgecolor="blue", hatch='---')
    pyplot.bar(indc, h0, wid, label="FIFO",
...
for i in range(0, 10):
    Throughput_c[i] *=  random.uniform(0.8,1.0)
    Throughput_d[i] *=  random.uniform(0.9,1.0)
    Throughput_b[i] *=  random.uniform(0.8,1.0)
    Throughput_x[i] *=  random.uniform(0.9,1.0)
drawHist_02(Throughput_c, Throughput_d, Throughput_b,
            Throughput_x, v_d, v_c, v_b, v_x, "newretrans")
