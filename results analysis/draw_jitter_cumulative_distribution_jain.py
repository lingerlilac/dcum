...
def drawHist_02(h0, h1, h2, h3, fname):
    ...
    pyplot.xlabel('Iteration')
    pyplot.ylabel('Overall Jitter (ms)')
    # pyplot.xlim(0, int(min(max(h0), max(h1))) - 1)
    pyplot.title('CDF of Jitters')
...
for key in keep_dic.keys():
    k1 = keep_dic[key][0][0]
    jitter_dic_cubic[k1] = compute_derivation(keep_dic[key], str(key))
...
for key in keep_dic.keys():
    k1 = keep_dic[key][0][0]
    jitter_dic_bbr[k1] = compute_derivation(keep_dic[key], str(key))
    # print(key, keep_dic[key])
del keep_dic, dic, del_set
if(f1):
    f1.close()
...
for key in keep_dic.keys():
    k1 = keep_dic[key][0][0]
    jitter_dic_nlapPF[k1] = compute_derivation(keep_dic[key], str(key))
    # print(key, keep_dic[key])
del keep_dic, dic, del_set
if(f1):
    f1.close()
...
A2_data_nlapPF = []
A3_data_nlapPF = []
for k in jitter_dic_nlapPF.keys():
    print(k, len(jitter_dic_nlapPF[k]))

for key in jitter_dic_nlapPF.keys():
    # print(key)
    if(key in A1):
        A1_data_nlapPF += jitter_dic_nlapPF[key]
    if(key in A2):
        A2_data_nlapPF += jitter_dic_nlapPF[key]
    if(key in A3):
        A3_data_nlapPF += jitter_dic_nlapPF[key]
del jitter_dic_nlapPF
gc.collect()
...
for key in keep_dic.keys():
    k1 = keep_dic[key][0][0]
    jitter_dic_xx[k1] = compute_derivation(keep_dic[key], str(key))
    # print(key, keep_dic[key])
del keep_dic, dic, del_set
if(f1):
    f1.close()
...
A2_data_xx = []
A3_data_xx = []
for k in jitter_dic_xx.keys():
    print(k, len(jitter_dic_xx[k]))

for key in jitter_dic_xx.keys():
    # print(key)
    if(key in A1):
        A1_data_xx += jitter_dic_xx[key]
    if(key in A2):
        A2_data_xx += jitter_dic_xx[key]
    if(key in A3):
        A3_data_xx += jitter_dic_xx[key]
del jitter_dic_xx
gc.collect()
...
A2_data_cubic = []
A3_data_cubic = []
# print(jitter_dic_bbr)
for key in jitter_dic_cubic.keys():
    # print(key)
    if(key in A1):
        A1_data_cubic += jitter_dic_cubic[key]
    if(key in A2):
        A2_data_cubic += jitter_dic_cubic[key]
    if(key in A3):
        A3_data_cubic += jitter_dic_cubic[key]
del jitter_dic_cubic
gc.collect()
...
A3_data_bbr = []
# print(jitter_dic_bbr)
for key in jitter_dic_bbr.keys():
    # print(key)
    if(key in A1):
        A1_data_bbr += jitter_dic_bbr[key]
    if(key in A2):
        A2_data_bbr += jitter_dic_bbr[key]
    if(key in A3):
        A3_data_bbr += jitter_dic_bbr[key]
del jitter_dic_bbr
gc.collect()

# drawCumulativeHist(A1_data_cubic, A1_data_bbr,
#                    A1_data_nlapPF, A1_data_xx, "A1jitter")
# drawCumulativeHist(A2_data_cubic, A2_data_bbr,
#                    A2_data_nlapPF, A2_data_xx, "A2jitter")
# drawCumulativeHist(A3_data_cubic, A3_data_bbr,
#                    A3_data_nlapPF, A3_data_xx, "A3jitter")
...
# drawCumulativeHist(data_cubic, data_bbr, data_nlapPF, data_xx, "overalljitter")

try:
    f1f = open('overalljitter.csv', 'w')
    f1w = csv.writer(f1f)
except Exception:
    print('overalljitter.csv openfailed')

length = len(data_cubic)
...
length_cubic = len(A1_data_cubic) + len(A2_data_cubic) + len(A3_data_cubic)
length_cubic = length_cubic
x1 = len(A1_data_cubic) / length_cubic
x2 = len(A2_data_cubic) / length_cubic
x3 = len(A3_data_cubic) / length_cubic
length_nlapPF = len(A1_data_nlapPF) + len(A2_data_nlapPF) + len(A3_data_nlapPF)
x1 = len(A1_data_nlapPF) / length_nlapPF
x2 = len(A2_data_nlapPF) / length_nlapPF
x3 = len(A3_data_nlapPF) / length_nlapPF
apallnlapPF = [0 for i in range(0, 10)]

for i in range(0, 10):
    apallnlapPF[i] = xnlapPF0[i] * x1 + xnlapPF1[i] * x2 + xnlapPF3[i] * x3
    v3[i] = f0[i] * x1 + f1[i] * x2 + f3[i] * x3
length_xx = len(A1_data_xx) + len(A2_data_xx) + len(A3_data_xx)
x1 = len(A1_data_xx) / length_xx
x2 = len(A2_data_xx) / length_xx
x3 = len(A3_data_xx) / length_xx
apallxx = [0 for i in range(0, 10)]

for i in range(0, 10):
    apallxx[i] = xxx0[i] * x1 + xxx1[i] * x2 + xxx3[i] * x3
    v4[i] = x0[i] * x1 + x1[i] * x2 + x3[i] * x3
apallcubic = [0 for i in range(0, 10)]
for i in range(0, 10):
    apallcubic[i] = xcubic1[i] * x1 + xcubic2[i] * x2 + xcubic3[i] * x3
    v1[i] = c1[i] * x1 + c2[i] * x2 + c3[i] * x3
length_bbr = len(A1_data_bbr) + len(A2_data_bbr) + len(A3_data_bbr)
x1 = len(A1_data_bbr) / length_bbr
x2 = len(A2_data_bbr) / length_bbr
x3 = len(A3_data_bbr) / length_bbr
apallbbr = [0 for i in range(0, 10)]
for i in range(0, 10):
    apallbbr[i] = xbbr0[i] * x1 + xbbr1[i] * x2 + xbbr3[i] * x3
    v2[i] = r0[i] * x1 + r1[i] * x2 + r3[i] * x3
# drawHist_02(apallcubic, apallbbr, apallnlapPF, apallxx, "iterationjitter")

try:
    f1f = open('jitter_cumulative.csv', 'w')
    f1w = csv.writer(f1f)
except Exception:
    print('jitter_cumulative.csv openfailed')

length = len(apallcubic)
for i in range(0, length):
...
gc.collect()
drawHist_01(APjain_c, APjain_b, APjain_d, APjain_e, "jainjitter")
try:
    f1f = open('jitter_jain.csv', 'w')
    f1w = csv.writer(f1f)
except Exception:
    print('jitter_jain.csv.csv openfailed')
...
if f1f:
    f1f.close()
# print(A1_data_cubic)
# print(jitter_dic_bbr.keys())
# print(jitter_dic_cubic.keys())
