...
def drawCumulativeHist(h0, h1, h2, h3, fname):
    ...
    pyplot.step(x, y, label="LRCC", color="orange")
    pyplot.step(x, y3, label="LRFL", color="green", linestyle="--")
    pyplot.step(x, y1, label="LR", color="blue", linestyle=":")
    pyplot.step(x, y2, label="RSSI", color="red", linestyle="-.")
...
try:
    ff = open('throughput.csv', 'w')
    fw = csv.writer(ff)
except Exception:
    print('throughput.csv open failed')
length = len(total_ratey1)
for i in range(0, length):
    datatmp = (total_ratey1[i], total_ratey2[i],
               total_ratey3[i], total_ratey4[i])
    fw.writerow(datatmp)
if ff:
    ff.close()
gc.collect()
