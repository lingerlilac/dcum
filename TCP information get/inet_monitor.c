...
----------------------------------------
Added by larry Lin: I'am very sorry, I don't remember
*/
...
int compare_now_last(struct data_kept *s, struct data_kept *t)
{
    int condition = 0;
    condition = s->idiag_sport == t->idiag_sport;
    condition += s->idiag_dport == t->idiag_dport;
    condition += s->tcpi_rcv_rtt == t->tcpi_rcv_rtt;
    condition += s->tcpi_unacked == t->tcpi_unacked;
    condition += s->tcpi_snd_cwnd == t->tcpi_snd_cwnd;
    if(condition == 5)
        return 1;
    else
        return 0;
...
void parse_diag_msg(struct inet_diag_msg *diag_msg, int rtalen, FILE * fp) {
    struct rtattr *attr;
    ...
    //     __be64 time = 0;
    //     struct timeval now;
    //     suseconds_t msec_1 = 0;
    //     time_t sec1 = 0;
    //     gettimeofday(&now, NULL);
    //     time = (__be64)now.tv_sec * 100000 + (__be64)now.tv_usec;
    //     fprintf(fp, "%llu, %s, %s, %d, %d,", time,
    //             local_addr_buf, remote_addr_buf, ntohs(diag_msg->id.idiag_sport), ntohs(diag_msg->id.idiag_dport));
    // }
    ...
    if (rtalen > 0) {
        ...
        while (RTA_OK(attr, rtalen)) {
            ...
            if (attr->rta_type == INET_DIAG_INFO) {
                ...
                {
                    char *p = NULL, *q = NULL;
                    char ip1[] = "192.168.111";
                    p = strstr(local_addr_buf, ip1);
                    q = strstr(remote_addr_buf, ip1);
                    if((p != NULL) && (q != NULL))
                    {
                        struct data_kept new_item;
                        memset(&new_item, 0, sizeof(struct data_kept));
                        new_item.idiag_sport = ntohs(diag_msg->id.idiag_sport);
                        new_item.idiag_dport = ntohs(diag_msg->id.idiag_dport);
                        new_item.tcpi_rcv_rtt = (double) tcpi->tcpi_rcv_rtt / 1000;
                        new_item.tcpi_unacked = tcpi->tcpi_unacked;
                        new_item.tcpi_snd_cwnd = tcpi->tcpi_snd_cwnd;

                        if(compare_now_last(&last_item, &new_item) == 0)
                        {
                            ...
                            gettimeofday(&now, NULL);
                            time = (__be64)now.tv_sec * 100000 + (__be64)now.tv_usec;
                            // printf("111: %s \t %s\n", local_addr_buf, remote_addr_buf);
                            fprintf(fp, "%llu, %d, %d, %g, %u, %u\n",
                                    time, ntohs(diag_msg->id.idiag_sport), ntohs(diag_msg->id.idiag_dport),
...
int main(int argc, char *argv[]) {
    ...
    memset(&last_item, 0, sizeof(struct data_kept));
    while (1)
    {
        ...
        while (1) {
            ...
            while (NLMSG_OK(nlh, numbytes)) {
                ...
                parse_diag_msg(diag_msg, rtalen, fp);
